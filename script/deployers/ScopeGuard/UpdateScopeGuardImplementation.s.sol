// SPDX-License-Identifier: Immuni Software PTE Ltd General Source License
// https://github.com/immunefi-team/vaults/blob/main/LICENSE.md
pragma solidity ^0.8.18;

// solhint-disable no-global-import
import "../../Base.s.sol";
import "openzeppelin-contracts/proxy/transparent/TransparentUpgradeableProxy.sol";
import "../../../src/guards/ScopeGuard.sol";
import "../../../src/proxy/ProxyAdminOwnable2Step.sol";

contract UpdateScopeGuardImplementation is BaseScript {
    function run() external broadcaster {
        address proxyAdminAddress = vm.envAddress("PROXY_ADMIN");
        address guardAddress = vm.envAddress("SCOPE_GUARD"); // proxy

        ProxyAdminOwnable2Step proxyAdmin = ProxyAdminOwnable2Step(proxyAdminAddress);

        address guardPrevOwner = ScopeGuard(guardAddress).owner();

        ScopeGuard newGuardImplementation = new ScopeGuard{ salt: "immunefi" }();
        TransparentUpgradeableProxy guardProxy = TransparentUpgradeableProxy(payable(guardAddress));
        proxyAdmin.upgrade(guardProxy, address(newGuardImplementation));

        require(ScopeGuard(guardAddress).owner() == guardPrevOwner, "ScopeGuard owner should not be changed");
    }
}
